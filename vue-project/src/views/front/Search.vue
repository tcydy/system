    ...
    <div style="width:80%;margin:0 auto">
        ...
        <div style="display:flex;gap:5px;margin-top:70px;padding:20px">
            ...
            <div v-for="item in types" :key="item.id" style="height:50px;">
                <el-card style="border-radius:50px" @click="changeType(item.id)":class="{'type-active':item.id == Number(typeId)}">
                    <span>{{ item.name }}</span>
                </el-card>
            </div>
        ...
        <div style="height: 400px;width: 100%;background-color: white;overflow: hidden;border:1px solid #e5e5e5;border-radius: 8px;box-shadow: 0 2px 8px rgba(0,0,0,0.04)" v-for="item in tableData" :key="item.id">
