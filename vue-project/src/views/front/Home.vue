...
const loadHotType =()=>{
  request.get('/type/hot').then(res=>{
    ...
    console.log(res);//测试
    //console.log(hotTypes.goodsList);
  })
}
    ...
    <div style="width:85%;margin:0 auto;height:100%;">
        ...
        <el-carousel height="50px" direction="vertical" :interval="3000" indicator-position="none">
          <el-carousel-item v-for="notice in notices" :key="notice.id">
            <div style="display:flex;margin-top: 20px;display: flex;justify-content: space-between;">
               <span style="color: #606266;cursor: default;">{{ notice.name }}:{{ notice.info }}</span>
               <span style="font-size:18px;color: grey;cursor: pointer;" @click="closeNotice">X</span>
            </div>
          </el-carousel-item>
        </el-carousel>
      </div>
      <div style="display: flex;gap:20px;height: 430px;padding: 10px">
        ...
        <div style="height: 100%;flex:1;padding: 10px;background-color: white;border:1px solid #dfdfdf;border-radius: 10px;">
          <div style="width:100%;height: 100%;display:flex;flex-direction: column;gap:18px;justify-content: center;align-items: center;">
            <div v-for="item in types":key="item.id" style="display: flex;" class="type-item">
              <div id="card-jump" style="display:flex;gap:10px;cursor: pointer;" @click="router.push('/front/search?typeId='+item.id)+'&keyword='+null">
        ...
        <div style="height: 100%;flex:2;border-radius: 10px;">
          <el-carousel height="410px">
            <el-carousel-item v-for="banner in banners" :key="banner.id">
             <div style="height: 100%;width: 100%;position: relative;cursor: default;">
        ...
        <div style="height: 100%;flex:5;display: grid;grid-template-columns: repeat(2,1fr);gap:20px">
          <div v-for="(item,index) in hotTypes":key="item.id" class="hotType-item":style="{background:categoryColors[index],gap:'5px'}">
            <!-- 分类信息 -->
            <div style="width:100px;display: flex; flex-direction: column; align-items: center;margin: auto;cursor: pointer;"
              @click="router.push('/front/search?typeId='+item.id)+'&keyword='+null">
              <div>
              ...
              <div style="margin-top:20px">
                <img :src="item.img" style="height:90px;width:90px;object-fit:cover" alt="">
              </div>
            </div>
            <!-- 分类右边的商品信息 -->
            <div style="margin: auto;height: 160px;background:rgba(255,255,255,0.8);width:70%;border-radius: 20px;padding:10px;display: grid;grid-template-columns: repeat(3, 1fr);gap: 5px;">
              <div v-for="goods in item.goodsList":key="goods.id" style="margin: auto;"  class="hotType-item-goods">
                <div style="flex:1;cursor: pointer;" @click="router.push('/front/goodsDetail?id='+goods.id)">
                  <div style="height:110px;width:100%">
                    <img :src="goods.img" style="width:100%;height: 100%;object-fit: cover;border-radius: 10px;" alt="">
                  </div>
                  <div style="text-align: center;margin-top: 5px;">
                    <span style="color: orangered;font-size: 16px;font-weight: 1000;">¥{{ goods.price }}</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  <div>
    <div style="width:80%;margin: 0 auto;height: 100%;">
      <div style="display: flex;justify-content: space-between;margin-top: 50px;margin-bottom: 20px">
        ...
        <div @click="router.push({path: '/front/search',query: {typeId: 0,keyword: undefined}})" style="cursor: pointer;">
          <span style="color:#3a76c4">查看全部</span>
        </div>
      </div>
      <!-- 下面的热门商品 -->
      <div style="display: grid;grid-template-columns:repeat(4,1fr);gap:20px;grid-auto-flow: row;">
        <div style="height: 380px;width: 100%;background-color: white;overflow: hidden;border:1px solid #e5e5e5;border-radius: 8px;box-shadow: 0 2px 8px rgba(0,0,0,0.04);cursor: pointer;" 
        v-for="item in goods" :key="item.id" @click="router.push('/front/goodsDetail?id=' + item.id)">
